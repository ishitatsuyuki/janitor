...
hostAPI.get({
  ...
  examples: [{
    ...
    response: {
      body: JSON.stringify({ port: '2376', ca: '', crt: '', key: '' })
    }
  }, {
    request: {
...
credentialsAPI.get({
  ...
  handler: (request, response) => {
    ...
    const host = hosts.get(request.params.hostname);
    if (!host) {
      response.statusCode = 404; // Not Found
      response.json({ error: 'Host not found' });
      return;
  ...
  examples: [{
    ...
    response: {
      body: JSON.stringify({ id: '1234', secret: '123456' })
    }
  }]
});
...
hostAPI.get('/version', {
  ...
  examples: [{
    ...
    response: {
      body: JSON.stringify({ docker: { Version: '17.06.0-ce' } })
    }
  }, {
    request: {
      urlParameters: { hostname: 'non-existent.example.com' }
...
containersAPI.put({
  ...
  handler (request, response) {
    ...
    machines.spawn(user, projectId, (error, machine) => {
      ...
      response.statusCode = 201; // Created
      response.json({ container: machine.docker.container });
    });
  },
...
containerAPI.patch({
  ...
  handler: (request, response) => {
    ...
    db.save();
    response.json(machine.properties);
  },
  ...
  examples: [{
    ...
    response: {
      body: JSON.stringify({ name: 'Much container. Such wow.' })
    }
  }, {
    request: {
      urlParameters: {
    ...
    response: {
      status: 400,
      body: JSON.stringify({ error: 'Invalid JSON Patch' })
    }
  }, {
    request: {
      urlParameters: {
    ...
    response: {
      status: 400,
      body: JSON.stringify({ error: 'Invalid container ID' })
    }
  }, {
    request: {
      urlParameters: {
  ...
  handler: (request, response) => {
    ...
    machines.destroy(user, machine.project, machine.id, error => {
      if (error) {
        ...
        response.statusCode = 500; // Internal Server Error
        response.json({ error: 'Could not delete container' });
        return;
      }
