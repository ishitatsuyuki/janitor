...
exports.get = function (req, res, callback) {
  ...
  const token = (req.cookies && req.cookies[cookieNames.token]) || '';
...
exports.destroy = function (request, response, callback) {
  exports.get(request, response, (error, session, token) => {
    if (error) {
      log('[fail] could not destroy the session', error);
    }

    // Destroy the cookie.
    response.clearCookie(cookieNames.token, {
      secure: useSecureCookies
    });
